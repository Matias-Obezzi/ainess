...
export function useActivityIsland() {
  useEffect(() => {
    return useAppStore.subscribe((state, prevState) => {
      const runningRuns = Object.values(state.runs).filter(r => r.status === "running" && r.projectId === state.currentProjectId);
      const prevRunningRuns = Object.values(prevState.runs).filter(r => r.status === "running" && r.projectId === prevState.currentProjectId);
