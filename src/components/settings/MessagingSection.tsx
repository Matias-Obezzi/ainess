...
//
// Nothing is exposed by turning this on: the app is the one that goes out and asks (Telegram) or
// keeps a socket open outward (Discord), so there is no port, no tunnel and no address for anyone
// to find. What there is instead is, per channel, a list of chats that may give orders — and that
// list is the whole of the security, which is why the card is mostly about filling it correctly
// and says out loud what it means.
...
const CHANNELS: ChannelSpec[] = [
  {
    id: "telegram",
    titleKey: "messaging.telegram.title",
    descriptionKey: "messaging.telegram.description",
    enableHintKey: "messaging.telegram.enableHint",
    tokenHintKey: "messaging.telegram.tokenHint",
  ...
  {
    id: "discord",
    titleKey: "messaging.discord.title",
    descriptionKey: "messaging.discord.description",
    enableHintKey: "messaging.discord.enableHint",
    tokenHintKey: "messaging.discord.tokenHint",
    tokenPlaceholder: "MTIzNDU2Nzg5MDEyMzQ1Njc4…",
...
export function MessagingSection() {
  return (
    <div className="flex flex-col gap-4">
      {CHANNELS.map(spec => <ChannelCard key={spec.id} spec={spec} />)}
    </div>
  );
}
...
function ChannelCard({ spec }: { spec: ChannelSpec }) {
  ...
  const channel = config.messaging?.[spec.id];
  ...
  // the connection is doing. A slow tick is enough for both and costs nothing while the panel is shut.
  const [unknown, setUnknown] = useState<string | null>(null);
  const [running, setRunning] = useState(false);

  useEffect(() => {
    const tick = () => {
      setUnknown(lastUnknownChatId(spec.id));
      setRunning(bridgeRunning(spec.id));
    };
    tick();
  ...
  const chats = channel?.allowedChatIds ?? [];

  const save = (patch: Partial<MessagingChannelConfig>) => {
    const messaging = config.messaging ?? {};
    updateConfig({
      messaging: {
        ...messaging,
        [spec.id]: { enabled: false, token: "", allowedChatIds: [], projectId: null, ...channel, ...patch },
      },
    });
    // A token or a list that changed mid-flight is only real once the channel reads it again.
    if (enabled) void restartBridge(spec.id);
  };
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex items-center gap-2">
          <Switch checked={enabled} onCheckedChange={checked => void toggleBridge(spec.id, checked)} />
          <div className="flex flex-col">
            <label className="text-sm font-semibold">{t("messaging.enable")}</label>
            <span className="text-sm text-muted-foreground">{t(spec.enableHintKey)}</span>
          </div>
          {enabled && (
        ...
        <div className="flex flex-col gap-1 border-t pt-3">
          ...
          <Input
            ...
            onBlur={() => token !== (channel?.token ?? "") && save({ token: token.trim() })}
          />
          <span className="text-sm text-muted-foreground">{t(spec.tokenHintKey)}</span>
          {spec.extraWarningKey && (
            <div className="flex items-center gap-2 rounded-md border border-amber-500/40 bg-amber-500/5 p-2">
              ...
              <span className="flex-1 text-xs">{t(spec.extraWarningKey)}</span>
            </div>
          )}
          <Button
            variant="ghost"
            ...
            onClick={() => void openExternal(spec.linkUrl)}
          >
        ...
        <div className="flex flex-col gap-1 border-t pt-3">
          ...
          <Select
            value={channel?.projectId ?? LAST_PROJECT}
