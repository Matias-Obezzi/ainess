...
- **Claude Code já não precisa de Node.js na máquina.** Ele se comunica por um adaptador publicado
  no npm e, até agora, a única porta era o `npx`: onde não havia Node.js não havia nada para
  iniciar e a execução morria com um erro de spawn que ninguém conseguia ler. Quando nem o adaptador
  nem o `npx` estão no PATH, o app agora monta o próprio ambiente: baixa um runtime e instala o
  adaptador na sua pasta de dados (~360 MB, uma única vez), atrás de uma tela que diz o que está
  fazendo e que pode ser repetida. Uma máquina que já tem Node.js continua usando o `npx` e não
  baixa nada.
- **Os seletores de modelo oferecem o que o CLI instalado consegue rodar de verdade.** Antigravity
  e opencode já eram consultados sobre seus modelos, mas só o diálogo do agente lia a resposta; o
  compositor, os presets, os chats e o anel de cota mostravam uma lista escrita no código, que
  envelhecia em silêncio. Agora todos os seletores leem a mesma lista: o que o CLI informa,
  atualizado ao iniciar e no máximo a cada dez minutos, com a lista embutida como reserva. O
...
- **O remoto do celular volta para onde você parou.** Reabrir ou recarregar a página (o que o
  ...
  instantâneo; um projeto ou chat que não existe mais volta ao início ou ao fio do orquestrador.
  (#28)
...
- **Dois processos para um mesmo agente.** A equipe é uma hierarquia com um único exemplar de cada
  agente, e nada fazia isso valer. Quando um implementador terminava enquanto o reviewer ainda
  trabalhava na tarefa que o planner lhe dera, a revisão começava mesmo assim — um segundo
  `agy.exe` sobre o mesmo reviewer, escrevendo a mesma conversa, enquanto os dois rodassem. Agora um
  agente é um único processo: o trabalho que chega a um agente no meio de um turno — uma delegação,
  uma revisão, a resposta à sua pergunta, uma nova tentativa — fica registrado como execução na
  fila e começa quando esse turno termina, na ordem em que chegou. O planner continua esperando por
  ela, o cartão do quadro sabe dela e o fio diz por quem está esperando. Parar o agente descarta
  também o que estava na fila dele.

- **O token do bot do Telegram estava sendo escrito no log.** Sempre que uma consulta ao Telegram
  falhava — a cada quarenta e cinco segundos, enquanto a rede estivesse fora — a URL inteira era
  registrada, e o Telegram carrega o token no caminho dessa URL:
  `api.telegram.org/bot<id>:<token>/getUpdates`. O mascarador conhecia `token=`, `Bearer` e
  `api_key`, e não essa forma. Agora conhece, dos dois lados do app, e nada que chega ao arquivo de
  log o carrega. **Se seus arquivos de log alguma vez saíram da sua máquina, revogue o token no
  BotFather e cole um novo** — o antigo está em cada `ainess-<data>.log` escrito antes desta
  versão.
...
- **O app fala sete idiomas em todo lugar, não só onde alguém lembrou.** Cento e sete frases estavam
  ...
  tarefa, e todo o CLI incluindo a tela de ajuda. Seis dos sete idiomas as recebiam num idioma que
  ninguém escolheu, e nada percebia — foi assim que chegaram a ser cem: cada uma era uma linha na
  hora.

  Não eram todas a mesma coisa. O que um usuário lê foi para os dicionários. O que só quem programa
  lê — todas as linhas de log — agora está em inglês: um log é grepado, colado num issue e lido por
  quem está depurando, e traduzir um torna-o inútil para todos menos a pessoa no idioma de quem ele
  ficou.

  A ajuda do CLI é uma única entrada por idioma e não vinte e quatro, porque suas colunas estão
  alinhadas e mantê-las alinhadas é uma decisão por idioma: o alemão precisa de mais espaço que o
  japonês, e duas dúzias de entradas soltas deixariam uma sair do alinhamento sem nada que mostrasse.

  E agora há algo que percebe: uma checagem que falha diante de um literal que se lê como prosa em
  espanhol fora de `src/i18n`, rodando como parte da suíte de testes. Ela procura espanhol e não
...
- **Perguntar ao Antigravity quanto resta não abre mais um terminal.** O app pega um console ao
  iniciar e o esconde, para que tudo o que um agente rodar por sua vez o herde e nada abra uma
  janela em nenhuma profundidade. As sondas curtas também o herdavam — e esconder um console depende
  de o `ShowWindow` alcançar a janela que o mostra, o que no Windows 11, onde o host de console
  padrão é o Windows Terminal num processo próprio, não acontece. Um filho que desenha um spinner
  levanta essa janela, e o `agy models` desenha um. Agora as sondas não recebem console: a mesma
